...
if __name__ == '__main__':
    ...
    eggnog_parser = parent_parser.add_mutually_exclusive_group(required=True)
    ...
    eggnog_parser.add_argument('--eggnog-data-dir',
                               help='eggnog data directory e.g. ~/m/db/eggnog-mapper/2.1.9')
    eggnog_parser.add_argument('--eggnog-annotation-file', help='eggnog .annotation file')

    kofam_parser = parent_parser.add_mutually_exclusive_group(required=True)
    ...
    with in_tempdir():
        ...
        if args.eggnog_annotation_file:
            ...
            raise NotImplementedError('untested, below code will fail')
            ...
            cog_annotations = read_multiple_annotations(
                [eggnog_data_dir],  # should be path to annotations$
        ...
        if args.kofam_tsv_file:
            ...
            extern.run(
                f'hmmsearch --tblout {hmmsearch_tblout} -o /dev/null --notextw --cpu {args.threads} {kofam_hmms_path} {protein_fasta}'
            )
    ...
    for model_path in args.models:
        ...
        results = pl.DataFrame({'node': args.protein_fasta, 'preds': preds})
