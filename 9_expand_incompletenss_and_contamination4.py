...
def remove_completeness_and_add_contamination(row, fraction_to_remove, fraction_to_add, cog_weights, cog_weight_indices):
...
if __name__ == '__main__':
    ...
    parent_parser.add_argument('--input-file', help='input file e.g. x_and_y_training_data_annotations.csv', required=True)
    parent_parser.add_argument('--output-file', help='output file e.g. completeness_removed_contamination_added_training_data.csv', required=True)
        ...
        for fraction_to_remove in fractions_to_remove:
            for fraction_to_add in fractions_to_add:
                to_add = pd.concat([
                    ...
                    remove_completeness_and_add_contamination(x_and_y.iloc[i, 1:], fraction_to_remove, fraction_to_add, cog_weights, cog_weight_indices)
                ])
